...
func init() {
	...
	AcHomeDir = fmt.Sprintf("%s/.ic", LocalUser.HomeDir)
	AcSaveFile = fmt.Sprintf("%s/maps", AcHomeDir)
	AcIdPubFile = fmt.Sprintf("%s/ic_id.pub", AcHomeDir)
	AcIdPrivFile = fmt.Sprintf("%s/ic_id", AcHomeDir)

	/* create our dir or nothing if it does exist already.. :) */
	os.MkdirAll(AcHomeDir, 0700)

	//icutl.DebugLog.Printf("Home: %p err: %s savefile: %p\n", LocalUser, err, AcSaveFile)
	//icutl.DebugLog.Printf("Home: %p (%s) Err: %s Savefile: %p (%s)\n", LocalUser, LocalUser.HomeDir, err, AcSaveFile, AcSaveFile)
}
...
func (psk *PSKMap) File2Map(infilestr string, passwd []byte) (bool, error) {
	icutl.DebugLog.Printf("File2Map CALL to  %s\n", infilestr)

	fileBuffer, err := ioutil.ReadFile(infilestr)
	if err != nil {
		icutl.DebugLog.Printf("load file read error: %s", err)
		return false, err
	}
	...
	if err != nil {
		icutl.DebugLog.Printf("load file json unmarshalling error: %s", err)
		return false, err
	}
...
func (psk *PSKMap) GetRDMapEntry(server string, channel string) ([]byte, bool) {
	icutl.DebugLog.Printf("===---=-=-=--==- GetRDMapEntry[@%p] (serv: %s channel: %s)! --==-=---=-=-=-==-\n", psk, server, channel)
...
func (psk *PSKMap) SetRDMapEntry(server, channel string, rnd []byte) {
	icutl.DebugLog.Printf("===---=-=-=--==- SetRDMapEntry[@%p] (serv: %s channel: %s)! --==-=---=-=-=-==-\n", psk, server, channel)
	/*
		if psk == nil {
			//(*psk) = make(PSKMap)
			icutl.DebugLog.Printf("===---=-=-=--==- Nil PSKMap! --==-=---=-=-=-==-\n")
			return &icutl.AcError{Value: -1, Msg: "Nil PSKMap", Err: nil}
		}
	*/
	rdmap, ok := psk.GetRDMap(server)
	if ok == true {
...
func (psk *PSKMap) GetRDMap(server string) (*RDMap, bool) {
	icutl.DebugLog.Printf("===---=-=-=--==- GetRDMap[@%p] (serv: %s)! --==-=---=-=-=-==-\n", psk, server)
	acm, ok := (*psk)[server]
	if ok == true {
		return acm.Rd, true
	}
...
func (psk *PSKMap) SetSKMapEntry(server string, channel string, sk *SecretKey) {
	icutl.DebugLog.Printf("===---=-=-=--==- SetSKMapEntry[@%p] (serv: %s channel: %s)! --==-=---=-=-=-==-\n", psk, server, channel)
...
func (psk *PSKMap) GetSKMap(server string) (*SKMap, bool) {
	icutl.DebugLog.Printf("===---=-=-=--==- GetSKMap[@%p] (serv: %s)! --==-=---=-=-=-==-\n", psk, server)
	acm, ok := (*psk)[server]
	if ok == true {
		return acm.Sk, true
	}
...
func (psk *PSKMap) GetPKMapEntry(server string, nick string) (*KexKey, bool) {
	icutl.DebugLog.Printf("===---=-=-=--==- GetPKMapEntry[@%p] (serv: %s nick: %s)! --==-=---=-=-=-==-\n", psk, server, nick)
...
func (psk *PSKMap) SetPKMapEntry(server string, nick string, pk *KexKey) {
	icutl.DebugLog.Printf("===---=-=-=--==- SetPKMapEntry[@%p] (serv: %s nick: %s)! --==-=---=-=-=-==-\n", psk, server, nick)
	/*
		if psk == nil {
			//(*psk) = make(PSKMap)
			//psk = NewPSKMap()
...
func (psk *PSKMap) DelPKMapEntry(server, nick string) bool {
	icutl.DebugLog.Printf("===---=-=-=--==- DelPKMapEntry[@%p] (serv: %s nick: %s)! --==-=---=-=-=-==-\n", psk, server, nick)
	pkmap, ok := psk.GetPKMap(server)
	if ok == true {
		_, ok := (*pkmap)[nick]
		if ok == true {
...
func (psk *PSKMap) GetPKMap(server string) (*PKMap, bool) {
	icutl.DebugLog.Printf("===---=-=-=--==- GetPKMap[@%p] (serv: %s)! --==-=---=-=-=-==-\n", psk, server)
	acm, ok := (*psk)[server]
	if ok == true {
		return acm.Pk, true
	}
